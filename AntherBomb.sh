...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing AntherBomb'
    else
        figlet AntherBomb
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mAnagh \e[0m"
    else
        echo -e "\e[1;34mCreated By \e[1;34m"
        toilet -f mono12 -F border Anagh
    fi
...
do
    ...
    if [ $ch -eq 1 ];then
        ...
        echo -e "\e[1;34m RUN AntherBombAgain..."
        pause
        exit
    elif [ $ch -eq 5 ];then
        banner
